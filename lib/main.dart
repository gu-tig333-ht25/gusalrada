  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title:
            const Text('TIG333 TODO', style: TextStyle(color: Colors.black87)),
        centerTitle: true,
            ...
            children: _visibleItems.map((item) {
              return ListTile(
                ...
                title: Text(
                  ...
                  style: TextStyle(
                    decoration:
                        (item["done"] as bool) ? TextDecoration.lineThrough : null,
                  ),
                ),
                trailing: IconButton(
...
class _AddTaskPageState extends State<AddTaskPage> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await context.read<TodoStore>().addItem(text);
      if (mounted) Navigator.pop(context, true);
    } catch (_) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Could not add. Try again.')),
        );
      }
    } finally {
